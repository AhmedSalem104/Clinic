...
test('role permissions keep clinical data away from reception', () => {
  ...
  assert.equal(hasPermission('reception', PERMISSIONS.VIEW_ALL_APPOINTMENTS), true);
  assert.equal(hasPermission('doctor', PERMISSIONS.VIEW_ASSIGNED_APPOINTMENTS), true);
  assert.equal(hasPermission('doctor', PERMISSIONS.MANAGE_BOOKINGS), false);
