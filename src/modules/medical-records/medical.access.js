...
const canAccessPatient = async (patientId, user) => {
  ...
  const result = await query(`SELECT TOP 1 1 AS Allowed FROM PatientAssignments WHERE PatientId=@patientId AND DoctorId=@doctorId AND EndedAt IS NULL`, (request) => request.input('patientId', sql.Int, patientId).input('doctorId', sql.Int, user.doctorId));
