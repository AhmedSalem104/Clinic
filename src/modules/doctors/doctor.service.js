...
const remove = async (id, req) => {
  ...
  const doctor = await repository.remove(id);
  if (!doctor) throw new AppError('الطبيب غير موجود.', 404, 'DOCTOR_NOT_FOUND');
  await recordAudit({ req, action: 'delete', entity: 'doctor', entityId: id, oldValue: before });
  return doctor;
};

module.exports = { ...repository, get, create, update, remove, setServices };
