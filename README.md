...
غيّر كلمات المرور فورًا في بيئة حقيقية. يضبط `TZ` افتراضيًا على `Africa/Cairo` حتى تتطابق جداول العمل مع توقيت العيادة، ويمكن تغييره من البيئة.
...
التطبيق لا يضع أسرارًا في الواجهة أو المستودع. الملفات الطبية تحفظ كـmetadata في SQL Server، بينما ملف التخزين قابل للنقل إلى object storage لاحقًا.
