...
تم تطبيق طبقة تحسينات تشغيلية للحفاظ على سرعة تحميل البيانات وسلامتها مع نمو عدد المرضى والحجوزات:

- طلبات البحث والبيانات تستخدم Server-side pagination، مع إلغاء طلب البحث السابق ومنع تكرار طلبات GET المتزامنة.
- كل طلب API له timeout واضح، والردود الطبية والمحمية تحمل `Cache-Control: no-store` لمنع تسريب البيانات عبر كاش مشترك.
- بيانات الحجز العامة الثابتة فقط لها كاش قصير؛ أما المواعيد المتاحة والطابور فيُطلبان دائمًا ببيانات حديثة.
- استعلامات المواعيد ولوحة اليوم تستخدم نطاقًا زمنيًا قابلًا للاستفادة من الفهارس بدل تحويل العمود في شرط البحث.
- أضيفت فهارس لمسارات البحث عالية الاستخدام: المواعيد حسب الوقت والحالة، جداول الأطباء، الاستثناءات، التوقفات، المستندات، التنبيهات وسجل التدقيق.
- اتصالات SQL Server تستخدم Connection Pool مركزيًا، مع `DB_CONNECTION_TIMEOUT_MS` و`DB_REQUEST_TIMEOUT_MS` لمنع الطلبات المعلقة.
- جلسة الدخول تستخدم HttpOnly/Secure cookies في الإنتاج وSameSite قابلًا للضبط، مع Rate Limiting على الدخول والـAPI وتتبع عدد الـproxy hops من البيئة.
- رفع المستندات يقيّد الحجم والامتداد وMIME type، ويتحقق من توقيع الملف الفعلي قبل حفظ Metadata.

بعد تغيير الفهارس أو إعدادات البيئة شغّل الاختبارات ثم طبّق الـschema idempotently:
