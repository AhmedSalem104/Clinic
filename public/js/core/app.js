...
const renderLogin = () => {
  ...
  const showPatientLogin = loginQuery.get('patient') === '1';
  const loginMode = showPatientLogin ? {
    label: 'حساب المريضة',
  ...
  document.querySelector('#login-form')?.addEventListener('submit', async (event) => {
    ...
    finally { button.disabled = false; button.innerHTML = `${loginMode.button} ${icon('arrow')}`; }
  });
  if (showStaff || showPatientLogin) document.querySelector('#login-email')?.focus();
};
