...
export const archiveButton = (attribute, id, label = 'حذف') => { const button = document.createElement('button'); button.type = 'button'; button.className = 'btn btn-danger text-xs'; button.dataset[attribute] = String(id); button.title = 'حذف آمن / أرشفة'; button.innerHTML = `${icon('close')} ${label}`; return button; };
