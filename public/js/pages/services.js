...
export async function render(outlet) {
  ...
  const load = async (page = 1) => {
    ...
    try {
      ...
      table.querySelectorAll('[data-toggle][data-next="false"]').forEach((toggle) => toggle.parentElement.append(archiveButton('archiveService', toggle.dataset.toggle, 'حذف')));
      table.querySelectorAll('[data-archive-service]').forEach((button) => button.addEventListener('click', async () => { if (!(await confirm('أرشفة الخدمة؟', 'لن تُستخدم في الحجوزات الجديدة، مع الاحتفاظ بالحجوزات السابقة.', 'حذف / أرشفة'))) return; try { await clinicService.deleteService(button.dataset.archiveService); toast('تمت أرشفة الخدمة'); await load(page); } catch (error) { window.Swal.fire({ icon: 'error', title: 'تعذر أرشفة الخدمة', text: error.message }); } }));
