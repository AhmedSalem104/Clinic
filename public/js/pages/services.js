...
const categories = [
  ['الكشف والاستشارات', 'الكشف والاستشارات'],
  ['متابعة الحمل', 'متابعة الحمل'],
  ['السونار', 'السونار'],
  ['مشاكل النساء', 'مشاكل النساء'],
  ['الخصوبة وتأخر الحمل', 'الخصوبة وتأخر الحمل'],
  ['تنظيم الأسرة', 'تنظيم الأسرة'],
  ['فحوصات وإجراءات', 'فحوصات وإجراءات'],
  ['خدمات ما بعد الولادة', 'خدمات ما بعد الولادة'],
  ['Consultation', 'استشارات (قديمة)'],
  ['Obstetrics', 'نساء وتوليد (قديمة)'],
  ['Imaging', 'تصوير (قديمة)']
];

const categoryOptions = (selected = '') => categories.map(([value, label]) => `<option value="${escapeHtml(value)}" ${value === selected ? 'selected' : ''}>${escapeHtml(label)}</option>`).join('');
const categoryTag = (category) => `<span class="service-category-tag">${escapeHtml(category || 'غير مصنف')}</span>`;

export async function render(outlet) {
  let editingId = null;
  outlet.innerHTML = `<div class="section-heading"><div><h1>${icon('tag')} الخدمات</h1><p>كتالوج عملي لخدمات النساء والتوليد والسونار والخصوبة وإجراءات العيادة.</p></div><button id="add-service" class="btn btn-primary">${icon('plus')} إضافة خدمة</button></div>
    <section class="service-catalog-hero"><div class="service-catalog-intro"><span class="service-catalog-icon">${icon('medical')}</span><div><span class="service-catalog-kicker">كتالوج العيادة</span><h2>خدمات واضحة وسهلة الاختيار</h2><p>كل خدمة لها تصنيف ومدة تشغيلية وقواعد حجز وطابور، لتظهر للمريضة وللاستقبال بشكل منظم.</p></div></div><div id="service-catalog-summary" class="service-catalog-summary"><div><strong>—</strong><span>إجمالي الخدمات</span></div><div><strong>—</strong><span>نشطة في العرض</span></div><div><strong>—</strong><span>متوسط المدة</span></div></div></section>
    <section id="service-editor" class="card mb-5 hidden p-5"><div class="mb-5 flex items-center justify-between"><div><h2 id="service-editor-title" class="text-lg font-semibold">إضافة خدمة</h2><p class="mt-1 text-xs text-slate-500">تغيير مدة الخدمة يؤثر على المواعيد الجديدة وحساب الانتظار.</p></div><button id="close-service-editor" class="btn btn-ghost" type="button" aria-label="إغلاق">${icon('close')}</button></div><form id="service-form"><div class="form-grid"><div><label class="form-label">اسم الخدمة <span class="text-red-500">*</span></label><input class="input" name="name" maxlength="160" required></div><div><label class="form-label">التصنيف</label><input class="input" name="category" list="service-categories" maxlength="100" placeholder="مثال: متابعة الحمل"><datalist id="service-categories">${categories.map(([value]) => `<option value="${escapeHtml(value)}"></option>`).join('')}</datalist></div><div><label class="form-label">المدة الأساسية بالدقائق <span class="text-red-500">*</span></label><input class="input" name="baseDurationMinutes" type="number" min="1" max="480" value="20" required></div><div><label class="form-label">الحالة</label><select class="select" name="isActive"><option value="true">نشطة</option><option value="false">غير نشطة</option></select></div><div class="flex items-center gap-5 pt-7"><label class="flex items-center gap-2 text-xs"><input type="checkbox" name="requiresQueue" checked> تحتاج طابور</label><label class="flex items-center gap-2 text-xs"><input type="checkbox" name="requiresBooking" checked> تحتاج حجز</label></div></div><div class="mt-5 flex justify-end gap-2"><button id="cancel-service" class="btn btn-secondary" type="button">إلغاء</button><button class="btn btn-primary" type="submit">${icon('save')} حفظ الخدمة</button></div></form></section>
    <section class="card"><div class="filter-bar border-b border-slate-100 p-4"><div class="relative min-w-[260px] flex-1"><span class="pointer-events-none absolute right-3 top-3 text-slate-400">${icon('search')}</span><input id="service-search" class="input pr-10" placeholder="ابحث باسم الخدمة أو التصنيف" aria-label="بحث الخدمات"></div><select id="service-category" class="select service-category-filter" aria-label="تصفية حسب التصنيف"><option value="">كل التصنيفات</option>${categoryOptions()}</select></div><div id="services-table" class="table-wrap p-1">${emptyState('جاري تحميل الخدمات...')}</div><div id="services-pagination" class="border-t border-slate-100 p-4 text-xs text-slate-500"></div></section>`;
  ...
  const summary = document.querySelector('#service-catalog-summary');
  ...
  category.addEventListener('change', () => load(1));
  await load(1);
}
