...
const renderRows = (rows) => rows.length ? `<table class="data-table"><thead><tr><th>المعرّف</th><th>الاسم</th><th>الهاتف</th><th>العمر</th><th>الطبيب الأساسي</th><th>الحالة الحالية</th><th>آخر زيارة</th><th>اكتمال الملف</th><th>الحالة</th><th></th></tr></thead><tbody>${rows.map((row) => `<tr><td><a class="page-link" href="/patients/${row.Id}" data-route="/patients/${row.Id}">${escapeHtml(row.PatientCode)}</a></td><td><a class="page-link" href="/patients/${row.Id}" data-route="/patients/${row.Id}">${escapeHtml(row.FullName)}</a></td><td dir="ltr" class="text-right">${escapeHtml(row.Phone)}</td><td>${row.DateOfBirth ? `${Math.floor((Date.now()-new Date(row.DateOfBirth).getTime())/31557600000)} سنة` : '—'}</td><td>${escapeHtml(row.AssignedDoctor || 'غير مسندة')}</td><td>${escapeHtml(row.CurrentCase || '—')}</td><td>${formatDate(row.LastVisit)}</td><td>${statusBadge(row.ProfileStatus || 'complete')}</td><td>${row.HighRiskFlag ? '<span class="badge badge-danger">! متابعة خاصة</span>' : statusBadge(row.Status)}</td><td><a class="btn btn-ghost text-xs" href="/patients/${row.Id}" data-route="/patients/${row.Id}">فتح الملف ${icon('arrow')}</a></td></tr>`).join('')}</tbody></table>` : emptyState('لا توجد مريضات مطابقة.', '<a class="btn btn-primary" href="/patients/new" data-route="/patients/new">إضافة أول مريضة</a>');
