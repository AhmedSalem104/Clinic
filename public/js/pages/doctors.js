...
export async function render(outlet) {
  ...
  const load = async (page = 1) => {
    ...
    try {
      ...
      table.querySelectorAll('[data-toggle]').forEach((toggle) => toggle.parentElement.append(deleteButton('deleteDoctor', toggle.dataset.toggle)));
      table.querySelectorAll('[data-delete-doctor]').forEach((button) => button.addEventListener('click', async () => { if (!(await confirm('حذف الطبيب نهائيًا؟', 'سيتم حذف حساب الطبيب وبيانات الإعدادات المرتبطة فقط إذا لم توجد حجوزات أو سجلات طبية مرتبطة. لا يمكن التراجع عن هذه العملية.', 'حذف نهائي'))) return; try { await clinicService.deleteDoctor(button.dataset.deleteDoctor); toast('تم حذف الطبيب نهائيًا'); await load(page); } catch (error) { window.Swal.fire({ icon: 'error', title: 'تعذر حذف الطبيب', text: error.message }); } }));
