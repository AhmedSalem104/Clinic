...
const allTabs = [
  ['overview', 'Overview'], ['visits', 'Visits'], ['cases', 'Cases'], ['pregnancy', 'Pregnancy'],
  ['medications', 'Medications'], ['allergies', 'Allergies'], ['labs', 'Labs'], ['ultrasound', 'Ultrasound'],
  ['documents', 'Documents'], ['progress', 'Progress'], ['reports', 'Reports'], ['timeline', 'Timeline']
];
let patientId;
...
export async function render(outlet, context) {
  ...
  try {
    ...
    outlet.innerHTML = `<div class="mb-5"><a class="page-link text-xs" href="/patients" data-route="/patients">← Back to patients</a></div><section class="card p-5"><div class="flex flex-wrap items-start justify-between gap-4"><div class="flex items-center gap-4"><div class="grid h-14 w-14 place-items-center rounded-full bg-blue-100 text-lg font-bold text-blue-700">${escapeHtml(patient.FullName?.split(' ').slice(0, 2).map((part) => part[0]).join(''))}</div><div><div class="flex items-center gap-2"><h1 class="text-xl font-bold">${escapeHtml(patient.FullName)}</h1>${patient.HighRiskFlag ? '<span class="badge badge-danger">! High Risk</span>' : ''}</div><div class="mt-2 text-xs text-slate-500">${escapeHtml(patient.PatientCode)} · ${escapeHtml(patient.Phone)} · ${patient.DateOfBirth ? formatDate(patient.DateOfBirth) : 'Date of birth not recorded'}</div><div class="mt-2 text-xs text-slate-500">Primary doctor: <strong>${escapeHtml(patient.AssignedDoctor || 'Not assigned')}</strong> · Case: <strong>${escapeHtml(patient.CurrentCase || 'None')}</strong></div></div></div><div class="flex gap-2"><a class="btn btn-secondary" href="/reports?patientId=${patient.Id}" data-route="/reports?patientId=${patient.Id}">Medical report</a><a class="btn btn-primary" href="/appointments/new?patientId=${patient.Id}" data-route="/appointments/new?patientId=${patient.Id}">Book appointment</a></div></div></section><div class="mt-5 flex gap-1 overflow-x-auto border-b border-slate-200">${visibleTabs.map(([id, label], index) => `<button class="tab ${index === 0 ? 'active' : ''}" data-tab="${id}">${label}</button>`).join('')}</div><div id="profile-tab-panel" class="mt-5"></div>`;
