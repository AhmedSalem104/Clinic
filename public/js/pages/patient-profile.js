...
const locationSummary = (patient) => {
  const latitude = Number(patient.LocationLatitude);
  const longitude = Number(patient.LocationLongitude);
  const hasCoordinates = Number.isFinite(latitude) && Number.isFinite(longitude);
  const mapLink = hasCoordinates ? ` <a class="page-link" href="https://www.google.com/maps?q=${encodeURIComponent(`${latitude},${longitude}`)}" target="_blank" rel="noreferrer">فتح الموقع</a>` : '';
  return `<div class="mt-2 text-xs text-slate-500">العنوان: <strong>${escapeHtml(patient.Address || 'غير مسجل')}</strong>${mapLink}</div>`;
};
