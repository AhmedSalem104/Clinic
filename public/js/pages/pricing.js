...
import { escapeHtml, emptyState, statusBadge, formatDate, formatMoney, toast, icon, loadingButton, confirm, deleteButton } from '../core/ui.js';
...
export async function render(outlet) {
  ...
  const load = async (page = 1) => {
    ...
    try {
      ...
      table.querySelectorAll('[data-toggle]').forEach((toggle) => toggle.parentElement.append(deleteButton('deletePricing', toggle.dataset.toggle)));
      table.querySelectorAll('[data-delete-pricing]').forEach((button) => button.addEventListener('click', async () => { if (!(await confirm('حذف السعر نهائيًا؟', 'سيتم حذف سجل السعر نهائيًا ولن يمكن استعادته.', 'حذف نهائي'))) return; try { await clinicService.deletePricing(button.dataset.deletePricing); toast('تم حذف السعر نهائيًا'); await load(page); } catch (error) { window.Swal.fire({ icon: 'error', title: 'تعذر حذف السعر', text: error.message }); } }));
